...
class CookieWishlist(CookieModel):
    def __init__(self, wishlist):
        ...
        books = Book.objects.filter(pk__in=wishlist.keys())
        self.item_count = len(books)
        for book in books:
            self.items.append(
                {
                    "book": get_book(book),
                    "date_added": wishlist[str(book.pk)]["date_added"],
                }
            )
...
class ModelData:
    ...
    def __init__(self, request):
        ...
        if self.user.is_authenticated:
            ...
            self.items = sorted(self.items, key=lambda item: -item["date_added"])
...
class OrderData(ModelData):
    ...
    def get_real(self, customer):
        ...
        for i in self.items:
            ...
            OrderItem.objects.create(book=book, order=order, quantity=i["quantity"])
        return order


class WishlistData(ModelData):
    def auth_init(self):
        self.wishlist, c = Wishlist.objects.get_or_create(customer=self.user.customer)
